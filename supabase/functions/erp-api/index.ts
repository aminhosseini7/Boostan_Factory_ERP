...
async function productAnalytics(user:AppUser,url:URL){
  ...
  if(performance.error||productsCostResponse.error||productsResponse.error)
    throw performance.error||productsCostResponse.error||productsResponse.error
  ...
  const rows=(productsResponse.data||[]).map((p:any)=>{
    ...
    return {
      productId:p.id, productName:p.name, weightKg:n(p.weight_kg),
      price:n(p.price), isActive:p.is_active,
      soldUnits:sold, returnUnits:returned, netUnits:netQty,
