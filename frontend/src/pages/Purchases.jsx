...
export default function Purchases(){
 const[rows,setRows]=useState([]),[products,setProducts]=useState([]),[materials,setMaterials]=useState([]),[form,setForm]=useState(blank),[error,setError]=useState(null),[ok,setOk]=useState(''),[busy,setBusy]=useState(false);
 async function load(){try{const[a,p,m]=await Promise.all([api.get('/purchases'),api.get('/products'),api.get('/materials')]);setRows(a.data);setProducts(p.data.filter(x=>x.isActive));setMaterials(m.data.filter(x=>x.isActive))}catch(e){setError(e)}}useEffect(()=>{load()},[]);
 const weighted=form.purchaseType!=='FINISHED_PRODUCT';
 async function submit(e){e.preventDefault();setError(null);setOk('');setBusy(true);try{await api.post('/purchases',{...form,quantity:toNumber(form.quantity),weightKg:toNumber(form.weightKg),unitPrice:toNumber(form.unitPrice)});setForm(blank);setOk('خرید ثبت شد و موجودی مربوطه به‌روزرسانی شد.');await load()}catch(e){setError(e)}finally{setBusy(false)}}
 ...
 {form.purchaseType==='FINISHED_PRODUCT'?<><label>محصول<select required value={form.productId} onChange={e=>setForm({...form,productId:e.target.value})}><option value="">انتخاب محصول</option>{products.map(x=><option key={x.id} value={x.id}>{x.name}</option>)}</select></label><label>تعداد (عدد)<input required type="text" inputMode="numeric" value={form.quantity} onChange={e=>setForm({...form,quantity:e.target.value})}/></label></>:<><label>وزن کل (کیلوگرم)<input required type="text" inputMode="decimal" value={form.weightKg} onChange={e=>setForm({...form,weightKg:e.target.value})}/></label><label>انبار مقصد<select value={form.materialId} onChange={e=>setForm({...form,materialId:e.target.value})}><option value="">انتخاب خودکار</option>{materials.map(x=><option key={x.materialId} value={x.materialId}>{x.name} — {formatNumber(x.stockKg)} کیلوگرم</option>)}</select></label></>}
 <label>{weighted?'قیمت هر کیلوگرم':'قیمت هر عدد'} (تومان)<input required type="text" inputMode="numeric" value={form.unitPrice} onChange={e=>setForm({...form,unitPrice:e.target.value})}/></label>
 <label className="wide">توضیحات<textarea value={form.note} onChange={e=>setForm({...form,note:e.target.value})}/></label>
 <div className="wide hint">همه وزن‌ها بر حسب کیلوگرم و همه قیمت‌ها بر حسب تومان هستند.</div>
