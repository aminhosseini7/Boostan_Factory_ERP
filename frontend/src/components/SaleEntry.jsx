...
export default function SaleEntry({products,customers,setCustomers,onSaved,busy,setBusy,setError,setOk}){
  ...
  const [plateParts,setPlateParts]=useState({first:'',letter:'',middle:'',city:''});
  ...
  async function createCustomer(){
    ...
    if(!/^\d{11}$/.test(normalizeDigits(newCustomer.phone.trim()))) {setError?.(new Error('شماره تماس مشتری باید ۱۱ رقم باشد.'));return}
  ...
  async function submit(e){
    e.preventDefault();if(submitting.current||busy)return;submitting.current=true;setError?.(null);setOk?.('');setBusy?.(true);
    try{
      ...
      const driverPhone=normalizeDigits(sale.driverPhone.trim());
      if(driverPhone&&!/^\d{11}$/.test(driverPhone))throw new Error('شماره تماس راننده باید ۱۱ رقم باشد.');
      const plate=plateToString(plateParts);
      if(plate&&!isCompletePlate(plateParts))throw new Error('پلاک را کامل وارد کنید: دو رقم، یک حرف، سه رقم و کد شهر دو رقمی.');
      const payload={paymentType:sale.paymentType,customerId:isCredit?sale.customerId:undefined,paymentAmount:isCredit?toNumber(sale.paymentAmount):undefined,paymentMethod:isCredit&&toNumber(sale.paymentAmount)>0?sale.paymentMethod:undefined,customerPayableAmount:finalTotal,driverName:sale.driverName.trim()||undefined,driverPhone:driverPhone||undefined,driverVehicle:plate||undefined,note:sale.note.trim()||undefined,items:sale.items.map(x=>({productId:x.productId,quantity:toNumber(x.quantity)}))};
      await api.post('/sales',payload);setSale(baseSale);setPlateParts({first:'',letter:'',middle:'',city:''});setManualTotal(false);setOk?.('فروش با موفقیت ثبت شد. تاریخ و ساعت به‌صورت خودکار ثبت شد.');try{await onSaved?.()}catch(_){setOk?.('فروش ثبت شد، اما به‌روزرسانی فهرست فروش ناموفق بود. صفحه را تازه‌سازی کنید.');}
  ...
  return <form className="panel operator-form" onSubmit={submit}>
    ...
    {isCredit&&<div className="credit-box">
      ...
      <div className="new-customer-box"><b>مشتری نسیه جدید</b><input placeholder="نام مشتری" value={newCustomer.name} onChange={e=>setNewCustomer({...newCustomer,name:e.target.value})}/><input placeholder="شماره تماس ۱۱ رقمی" inputMode="numeric" maxLength={11} dir="ltr" value={newCustomer.phone} onChange={e=>setNewCustomer({...newCustomer,phone:normalizeDigits(e.target.value).replace(/\D/g,'').slice(0,11)})}/><button type="button" className="ghost" onClick={createCustomer}>ثبت مشتری</button></div>
      <div className="form-grid"><label>پیش‌پرداخت (اختیاری)<NumericInput value={sale.paymentAmount} onChange={e=>setSale({...sale,paymentAmount:e.target.value})}/></label>{toNumber(sale.paymentAmount)>0&&<label>روش پیش‌پرداخت<select value={sale.paymentMethod} onChange={e=>setSale({...sale,paymentMethod:e.target.value})}><option value="CASH">نقدی</option><option value="CARD">کارت</option><option value="BANK_TRANSFER">انتقال بانکی</option><option value="CHECK">چک</option><option value="OTHER">سایر</option></select></label>}<div className="debt-preview">مانده نسیه پس از ثبت: <b>{formatToman(Math.max(0,finalTotal-toNumber(sale.paymentAmount)))}</b></div></div>
    </div>}
    <h4>اقلام فروش</h4>
    {sale.items.map((it,i)=><div className="sale-line" key={i}><select required value={it.productId} onChange={e=>setItem(i,'productId',e.target.value)}><option value="">انتخاب محصول</option>{products.map(x=><option key={x.id} value={x.id}>{x.name} — {formatToman(x.price)}</option>)}</select><NumericInput required value={it.quantity} onChange={e=>setItem(i,'quantity',e.target.value)}/>{sale.items.length>1&&<button type="button" className="small danger" onClick={()=>removeItem(i)}>حذف</button>}</div>)}
    <button type="button" className="ghost" onClick={addItem}>افزودن قلم دیگر</button>
    <div className="sale-total"><div>مبلغ محاسبه‌شده: <b>{formatToman(subtotal)}</b></div><label>مبلغ نهایی فروش به مشتری<NumericInput value={sale.customerPayableAmount} onChange={e=>{setManualTotal(true);setSale({...sale,customerPayableAmount:e.target.value})}}/></label><div>تخفیف محاسبه‌شده: <b>{formatToman(discount)}</b></div></div>
    <div className="driver-box"><h4>اطلاعات راننده (اختیاری)</h4><div className="form-grid"><label>نام راننده<input value={sale.driverName} onChange={e=>setSale({...sale,driverName:e.target.value})}/></label><label>شماره تماس<input inputMode="numeric" maxLength={11} dir="ltr" placeholder="شماره تماس ۱۱ رقمی" value={sale.driverPhone} onChange={e=>setSale({...sale,driverPhone:normalizeDigits(e.target.value).replace(/\D/g,'').slice(0,11)})}/></label><div className="plate-field-label"><span>پلاک خودرو (اختیاری)</span><IranPlateInput value={plateParts} onChange={setPlateParts}/></div></div></div>
