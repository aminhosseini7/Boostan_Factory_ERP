...
export function jalaliToGregorianDate(value){const raw=normalizeDigits(value).trim().replace(/-/g,'/');const m=raw.match(/^(\d{3,4})\/(\d{1,2})\/(\d{1,2})$/);if(!m)throw new Error('تاریخ را به شکل ۱۴۰۵/۰۶/۲۸ وارد کنید');const jy=Number(m[1]),jm=Number(m[2]),jd=Number(m[3]);if(jm<1||jm>12||jd<1||jd>31||(jm>6&&jd>30))throw new Error('تاریخ جلالی معتبر نیست');const g=toGregorian(jy,jm,jd);return `${String(g.gy).padStart(4,'0')}-${String(g.gm).padStart(2,'0')}-${String(g.gd).padStart(2,'0')}`}
